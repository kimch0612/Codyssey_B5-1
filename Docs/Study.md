...
### 첫 번째 구현 단위: 빈 힙의 저장 상태와 `size`

**대응: CHECK 4단계의 저장 형태 및 `size` 기반 상태 확인 / 근거: Subject 4장 「힙」**

이번 단위에서는 `MinHeap.__init__(self) -> None`과 `size(self) -> int`만 구현한다. 호출 흐름은 **`MinHeap()` 생성 → 인스턴스 내부에 빈 배열 준비 → `size()` 호출 → 내부 배열의 현재 길이 반환**이다. 아직 항목을 넣거나 힙 순서를 복구하지 않는다.

#### 내부 배열과 항목 구분하기

| 표현 | 타입 | 의미 |
| --- | --- | --- |
| 힙 인스턴스 | `MinHeap` | 내부 배열과 앞으로 구현할 힙 연산을 관리하는 객체 |
| `self._items` | `List[Tuple[float, str]]` | 모든 힙 항목을 완전 이진 트리 순서로 담는 내부 Python 리스트 |
| 배열의 한 칸 | `Tuple[float, str]` | `(expire_at, key)` 형태의 힙 항목 |
| `item[0]` | `float` | 비교 기준인 만료 시각 |
| `item[1]` | `str` | 만료 대상인 저장소 키 |

예를 들어 `(80.0, "user:2")`는 하나의 튜플 객체다. `80.0`은 만료 시각이고 `"user:2"`는 키다. 이 튜플이 내부 배열의 한 칸에 저장되는 것이며, 배열 자체와 한 항목을 같은 대상으로 생각하지 않는다.

타입 힌트의 `float`는 만료 시각을 비교할 수 있는 수치로 표현하기 위한 선택이다. Python에서는 정수 시각도 이 연산에 사용할 수 있다. 실제 시각을 어떤 기준으로 만들고 남은 초를 어떻게 계산할지는 TTL 단계의 별도 결정이며, 지금 확정하지 않는다.

#### 왜 Python 리스트를 사용해도 직접 구현한 힙인가

Python 리스트는 여기서 인덱스로 부모·자식 위치에 접근하는 **저장 공간**이다. 리스트에 항목이 들어 있다고 자동으로 최소 힙 조건이 만들어지는 것은 아니다. 이후 `_heapify_up`과 `_heapify_down`에서 비교·교환 절차를 직접 구현해야 한다.

Subject의 동적 배열 직접 구현은 보너스이므로 이번 필수 범위에서 제외한다. 따라서 리스트의 항목 추가·마지막 항목 제거 같은 배열 기능은 사용할 수 있지만, `heapq` 같은 내장 힙 API나 매번 전체 배열을 정렬하는 방식으로 `push`·`pop`을 대신하면 안 된다.

#### 빈 상태와 인스턴스별 독립성

새 `MinHeap`의 `self._items`는 빈 리스트이고 `size()` 결과는 `0`이다. 두 힙 인스턴스를 만들었을 때는 서로 다른 내부 리스트를 가져야 한다. 한 인스턴스의 항목이 다른 인스턴스에 나타나면 안 되므로, 내부 배열은 클래스 전체나 기본 인자가 아니라 각 인스턴스의 생성 과정에서 만들어야 한다.

`size()`는 내부 배열의 길이를 읽기만 한다. 항목을 추가·삭제하거나 배열을 새로 만들지 않으며, Python 리스트의 길이 조회는 O(1)이다. 별도 `_size` 속성을 두는 설계도 가능하지만, 이번에는 배열 길이와 별도 개수가 어긋날 가능성을 만들지 않기 위해 길이를 직접 반환한다.

**이번 단위의 확인 기준:** 새 힙의 내부 배열이 비어 있고 `size()`가 정수 `0`을 반환해야 한다. `size()`를 반복 호출해도 같은 내부 배열 객체와 내용이 유지되어야 하며, 서로 다른 두 `MinHeap` 인스턴스의 내부 배열은 독립적이어야 한다. 아직 `push`가 없으므로 항목 삽입이나 최소 힙 조건은 이번 확인 범위가 아니고, 구현 전에는 CHECK 항목을 완료로 표시하지 않는다.

### 두 번째 구현 단위: `push`와 `_heapify_up`

**대응: CHECK 4-1의 부모·자식 관계, 위쪽 복구, 삽입 / 근거: Subject 4장 「힙」**

이번 단위에는 `_heapify_up(self, index: int) -> None`과 `push(self, item: Tuple[float, str]) -> None`를 구현한다. 호출 흐름은 **`push(item)` 호출 → `self._items` 끝에 항목 추가 → 새 항목의 마지막 인덱스 계산 → `_heapify_up(index)` 호출 → 부모보다 만료 시각이 작으면 튜플 전체 교환 → 새 위치에서 다시 부모와 비교**다.

#### 배열 인덱스로 보는 부모와 자식

힙에는 별도의 `Node` 객체나 `prev`·`next` 참조가 없다. 배열 인덱스가 트리 위치를 나타낸다.

| 현재 인덱스 `i` | 부모 | 왼쪽 자식 | 오른쪽 자식 |
| ---: | ---: | ---: | ---: |
| 일반식 | `(i - 1) // 2` | `2 * i + 1` | `2 * i + 2` |
| `0` | 없음 | `1` | `2` |
| `1` | `0` | `3` | `4` |
| `2` | `0` | `5` | `6` |

`_heapify_up`에서는 현재 항목에서 부모 방향으로만 이동하므로 부모 공식만 사용한다. 현재 인덱스가 `0`이면 이미 루트라서 부모가 없고 반복을 끝낸다.

#### 지역 인덱스와 저장된 항목 구분하기

`index`와 `parent_index`는 배열 위치를 나타내는 지역 정수다. `self._items[index]`와 `self._items[parent_index]`는 각각 `(expire_at, key)` 튜플이다. 비교할 때는 두 튜플의 첫 번째 값인 만료 시각을 사용하지만, 위치를 바꿀 때는 튜플 전체를 교환해야 한다.

예를 들어 `(70.0, "d")`가 위로 올라가야 할 때 `70.0`만 교환하고 키 `"d"`를 그대로 두면 시각과 키의 짝이 깨진다. 배열 한 칸의 항목 전체가 하나의 TTL 기록이라는 점을 유지한다.

#### 실제 삽입과 상태 변화

다음은 만료 시각을 차례로 100, 80, 120, 70으로 넣는 과정이다.

| 호출 | 배열 끝에 추가한 직후 | 위쪽 복구 후 |
| --- | --- | --- |
| `push((100.0, "a"))` | `[(100, a)]` | 루트이므로 그대로 |
| `push((80.0, "b"))` | `[(100, a), (80, b)]` | `[(80, b), (100, a)]` |
| `push((120.0, "c"))` | `[(80, b), (100, a), (120, c)]` | 부모 80보다 크므로 그대로 |
| `push((70.0, "d"))` | `[(80, b), (100, a), (120, c), (70, d)]` | `[(70, d), (80, b), (120, c), (100, a)]` |

마지막 항목은 처음에 인덱스 3이다. 부모 인덱스는 1이고 만료 시각 70이 100보다 작으므로 교환한다. 이제 현재 인덱스는 1이고 새 부모 인덱스는 0이다. 70이 80보다 작으므로 다시 교환해 루트에 도달한다.

#### 반복을 멈추는 두 조건

위쪽 복구는 다음 중 하나가 되면 끝난다.

1. 현재 인덱스가 `0`이어서 부모가 없다.
2. 현재 항목의 만료 시각이 부모의 만료 시각보다 작지 않다.

두 번째 조건에는 같은 만료 시각도 포함된다. 이번 설계에서는 동률이면 교환하지 않는다. 같은 시각 항목이 여러 개 있어도 모두 배열에 남아 있어야 하지만, 같은 시각 안의 특정 반환 순서는 과제에서 요구하지 않는다.

`push`가 항목을 추가한 뒤 `_heapify_up`을 호출하지 않거나, 한 번만 비교하고 끝내면 새 최소값이 여러 층을 올라가야 하는 경우 루트까지 도달하지 못한다. 교환 후 현재 인덱스를 부모 인덱스로 갱신해 다시 비교해야 한다.

#### 시간 복잡도

완전 이진 트리의 높이는 원소 수를 `n`이라 할 때 O(log n)이다. 새 항목은 한 층마다 부모와 비교하며 위로 이동하므로 `_heapify_up`은 최악의 경우 O(log n)이다. 리스트 끝 추가의 상각 O(1)을 합쳐도 `push`의 지배적인 비용은 O(log n)이다.

**이번 단위의 확인 기준:** 항목 하나와 이미 큰 순서·작은 순서·섞인 순서로 여러 항목을 넣어, 매 삽입 후 모든 부모의 만료 시각이 자식보다 크지 않아야 한다. 가장 작은 만료 시각이 인덱스 0에 있고, 같은 시각의 서로 다른 키도 유실되지 않아야 한다. `size()`는 삽입 수와 일치하고, `push`는 `None`을 반환해야 한다. 내장 힙 API나 전체 정렬을 사용하지 않아야 하며, 구현 전에는 관련 CHECK 항목을 완료로 표시하지 않는다.

### 세 번째 구현 단위: 최소 항목을 보는 `peek`

**대응: CHECK 4-2의 `peek` 및 빈 힙 규칙 / 근거: Subject 4장 「힙」**

이번 메서드는 `peek(self) -> Optional[Tuple[float, str]]`다. 호출 흐름은 **내부 배열이 비었는지 확인 → 비었으면 `None` 반환 → 항목이 있으면 인덱스 0의 튜플 반환**이다. 배열에서 항목을 빼거나 위치를 바꾸지 않는다.

`Optional[Tuple[float, str]]`는 반환값이 `(expire_at, key)` 튜플이거나 `None`이라는 뜻이다. 예를 들어 내부 배열이 다음과 같다면:

```text
[(70.0, "d"), (80.0, "b"), (120.0, "c"), (100.0, "a")]
```

`peek()`은 `(70.0, "d")`를 반환한다. 호출 후에도 배열은 그대로이고 `size()`는 `4`다. 반환되는 튜플은 지역 변수에 담을 수 있지만, 그것을 반환한다고 내부 배열에서 제거되는 것은 아니다.

#### 왜 인덱스 0만 보면 되는가

`push`와 `_heapify_up`이 최소 힙 조건을 유지하므로 모든 자식은 부모보다 먼저 만료되지 않는다. 따라서 루트보다 더 작은 만료 시각이 아래쪽에 있을 수 없고, 전체 배열을 순회하거나 정렬할 필요 없이 `_items[0]`만 읽으면 된다. 이 조회는 O(1)이다.

`peek`이 반환하는 것은 실제 저장 값이 아니라 만료 후보 튜플이다. 예를 들어 `(70.0, "user:2")`를 받으면 후속 TTL 처리 코드가 시각을 현재 시각과 비교하고, 실제 값이나 키 존재 여부는 해시맵에서 확인한다.

#### 빈 배열을 먼저 확인하는 이유

빈 리스트에서 `_items[0]`에 바로 접근하면 `IndexError`가 발생한다. 이번 내부 API에서는 빈 힙을 정상 상태로 보고 `None`을 반환하기로 정한다. 이는 나중에 CLI에서 출력할 `(nil)` 문자열과 같은 것이 아니라, Python 내부 메서드의 반환 규칙이다.

조건을 확인하기 위해 `size()`를 사용하거나 리스트의 참·거짓을 확인할 수 있다. 어느 쪽이든 빈 상태 확인 뒤에만 인덱스 0에 접근해야 한다. 조회 전후 배열 객체·항목 순서·개수는 모두 같아야 한다.

**이번 단위의 확인 기준:** 빈 힙에서는 `None`, 항목 하나 이상에서는 가장 작은 만료 시각을 가진 루트 튜플을 반환해야 한다. 반복 호출해도 같은 결과이며, `_items`의 객체·순서·항목 수와 `size()`가 변하지 않아야 한다. 아직 `pop`은 구현하지 않으므로 제거와 아래쪽 복구는 이번 확인 범위가 아니고, 구현 전에는 CHECK 항목을 완료로 표시하지 않는다.

### 네 번째 구현 단위: `pop`과 `_heapify_down`

**대응: CHECK 4-2의 최소 항목 제거, 아래쪽 복구, 빈 힙 규칙 / 근거: Subject 4장 「힙」**

이번 단위에는 `_heapify_down(self, index: int) -> None`과 `pop(self) -> Optional[Tuple[float, str]]`를 구현한다. 호출 흐름은 **빈 힙 확인 → 기존 루트 항목 보관 → 마지막 항목 제거 → 항목이 남았다면 마지막 항목을 루트에 배치 → `_heapify_down(0)` 호출 → 보관한 기존 루트 반환**이다.

#### 루트 자리를 마지막 항목으로 채우는 이유

배열 0번만 제거하면 맨 앞에 빈칸이 생기며, 뒤의 모든 항목을 한 칸씩 당기면 O(n)이 든다. 배열의 마지막 항목은 완전 이진 트리의 가장 마지막 위치이므로 O(1)에 제거할 수 있다. 이 항목을 빈 루트 자리에 놓으면 배열의 연속된 인덱스와 완전 이진 트리 모양은 유지된다.

다만 마지막 항목은 새 루트의 자식보다 만료 시각이 늦을 수 있다. 그래서 삽입 때와 반대 방향인 `_heapify_down`으로 아래쪽 관계를 복구한다.
...
| 단계 | 보관한 최소 항목 | 내부 배열 |
| --- | --- | --- |
| 시작 | 없음 | `[(70,d), (80,b), (120,c), (100,a)]` |
| 루트 보관 | `(70,d)` | 그대로 |
| 마지막 항목 제거 | `(70,d)` | `[(70,d), (80,b), (120,c)]`, 제거한 항목은 `(100,a)` |
| 마지막 항목을 루트에 배치 | `(70,d)` | `[(100,a), (80,b), (120,c)]` |
| 아래쪽 복구 | `(70,d)` | `[(80,b), (100,a), (120,c)]` |
| 반환 | `(70,d)` | 복구된 배열 유지 |

반환할 대상은 처음 루트였던 `(70.0, "d")`다. 루트로 옮길 대상은 배열 끝에서 제거한 `(100.0, "a")`다. 두 지역 변수를 혼동하면 최소 항목 대신 마지막 항목을 반환할 수 있다.

#### 두 자식 중 먼저 비교할 자식 고르기

현재 위치를 `index`라 하면 자식 인덱스는 다음과 같다.
...
아래쪽 복구는 먼저 존재하는 자식 중 만료 시각이 더 작은 자식을 고른다. 왼쪽만 있으면 왼쪽을 선택하고, 둘 다 있으면 두 만료 시각을 비교한다. 이번 설계에서는 오른쪽이 왼쪽보다 **엄격히 작을 때만** 오른쪽을 고르므로 동률이면 왼쪽이 선택된다.

두 자식 중 작은 쪽을 선택하지 않고 무조건 왼쪽과만 교환하면, 오른쪽 자식이 더 작을 때 새 부모가 오른쪽보다 커져 힙 조건이 계속 깨진 상태가 될 수 있다.

#### 아래쪽 반복의 종료 조건

`_heapify_down`은 다음 중 하나가 되면 끝난다.

1. 왼쪽 자식 인덱스가 항목 수 이상이라 자식이 없다.
2. 현재 항목의 만료 시각이 선택한 더 작은 자식보다 크지 않다.

교환했다면 현재 위치를 선택한 자식 인덱스로 바꾸고, 그 위치의 자식들을 다시 계산한다. 인덱스를 갱신하지 않으면 한 층만 내려가거나 같은 위치를 반복해서 확인하게 된다.

#### 빈 힙과 한 항목 힙

빈 힙의 `pop()`은 `None`을 반환하며 상태를 유지한다. 항목이 하나라면 그 루트가 동시에 마지막 항목이다. 마지막 항목을 제거한 뒤 배열이 비므로 인덱스 0에 다시 대입하거나 `_heapify_down(0)`을 호출하지 않고, 보관한 항목을 바로 반환한다.

두 경우를 먼저 분리하지 않으면 빈 배열의 0번에 접근하거나, 하나뿐인 항목을 제거한 다음 빈 배열의 0번에 대입해 `IndexError`가 발생할 수 있다.

#### 시간 복잡도

마지막 항목 제거와 루트 대입은 O(1)이다. `_heapify_down`은 완전 이진 트리의 높이만큼 내려가므로 최악의 경우 O(log n)이며, `pop` 전체도 O(log n)이다. 전체 배열을 정렬하거나 모든 항목을 순회해 다음 최소값을 찾지 않는다.

**이번 단위의 확인 기준:** 빈 힙과 한 항목 힙, 왼쪽 자식만 있는 경우, 오른쪽 자식이 더 작은 경우, 여러 층 내려가는 경우를 확인한다. 섞인 순서와 같은 만료 시각의 항목을 넣은 뒤 반복 `pop()`하면 만료 시각이 감소하지 않는 순서로 모두 반환되어야 한다. `peek()`은 개수를 유지하고 `pop()`은 성공할 때마다 하나만 줄여야 하며, 삽입과 제거를 번갈아 해도 힙 조건이 유지되어야 한다. 내장 힙 API나 전체 정렬을 사용하지 않아야 하며, 구현 전에는 남은 CHECK 항목을 완료로 표시하지 않는다.
...
### 6-2. 삭제·존재·전체 조회 (CHECK 5단계, 5-2)

**근거: Subject 2장 「String 타입 명령어」, 평가 1-1 / CHECK 5단계 5-2**

`Store`의 네 가지 메서드는 모두 내부 `HashMap`의 공개 메서드를 호출하는 구조다.

| Store 메서드 | 호출하는 HashMap 메서드 | 반환 |
| --- | --- | --- |
| `del_key(key)` | `remove(key)` | 삭제 성공 시 `True`, 없는 키이면 상태를 유지하며 `False` |
| `exists(key)` | `contains(key)` | 키 존재 여부 `bool` |
| `dsize()` | `size()` | 저장된 서로 다른 키 수 `int` |
| `keys()` | `keys()` | 저장된 키 문자열을 담은 목록 |

**`exists`와 `get`의 차이**: `get("n")`는 값을 반환하므로 저장된 값이 `None`이면 "없는 키"와 구별할 수 없다. `contains`는 값의 참·거짓이 아니라 일치하는 노드의 유무로 `bool`을 반환한다. 따라서 "키가 있는가"를 묻는 `EXISTS` 명령은 `contains`에 기반한다.

**`del_key`의 동작**: `remove`가 노드를 찾아 버킷에서 분리하고 `_size`를 1 줄인다. 같은 키를 두 번 삭제하면 첫 번째만 `True`, 두 번째는 `False`다. 삭제 후 `get`은 `None`을 반환해야 한다.

**`keys`의 동작**: 모든 버킷을 순회해 노드의 `data.key`를 모은다. 정렬과 삽입 순서 보장은 요구하지 않으며, 패턴 매칭도 추가하지 않는다.

**구분할 점**: 내부 메서드의 반환값(`True`/`False`, `int`, 키 목록)과 CLI가 출력하는 `(integer) 1`, `(integer) 0`, 키 배열 문자열은 별개다. 출력 형식은 10단계에서 적용한다.

**스스로 설명해 보기**: `set("n", None)` 후 `exists("n")`이 `True`여야 하는 이유는 무엇인가? `dsize()`가 `len(keys())`과 항상 같아야 하는 근거를 해시맵 구조에서 설명해보자.
...
### 선택한 내부 구조: 해시맵에서 LRU 노드로 직접 접근

이번 구현에서는 **LRU 리스트의 앞(`head`)을 MRU, 뒤(`tail`)를 LRU**로 정한다. 반대 방향도 가능한 설계이지만, 이 프로젝트에서는 앞쪽 삽입·이동과 뒤쪽 제거를 조합하는 방향을 선택했다.

문자열 키와 값은 작은 `StoreEntry` 객체로 묶고, LRU 리스트의 `Node.data`가 그 객체를 보관한다. 저장소의 해시맵에는 값 문자열 자체가 아니라 해당 LRU `Node`를 저장한다.

| 객체 또는 참조 | 실제로 보관하는 것 |
| --- | --- |
| `StoreEntry` | 문자열 `key`와 `value` |
| LRU 리스트의 `Node.data` | `StoreEntry` 객체 |
| 저장소의 `HashMap` | `key → LRU Node` |
| `self._lru.head` | 가장 최근에 사용한 엔트리(MRU) |
| `self._lru.tail` | 가장 오래 사용하지 않은 엔트리(LRU) |

여기서 **해시맵 버킷의 노드와 LRU 리스트의 노드는 서로 다른 객체**다. 해시맵 버킷 노드의 `data`는 기존 `HashEntry`이고, 그 `HashEntry.value`가 LRU 노드를 참조한다. LRU 노드의 `data`는 `StoreEntry`다. 하나의 `Node`가 두 리스트에 동시에 연결되는 것이 아니므로 체이닝 순서와 사용 순서의 `prev`·`next`가 서로를 덮어쓰지 않는다.
...
새 키를 저장할 때는 `StoreEntry`를 먼저 만들고 `self._lru.insert_front(entry)`에 전달한다. `insert_front`가 새 LRU 노드를 만들어 반환하면 그 노드를 해시맵에 저장한다. 현재 연결 리스트의 `insert_front`는 **데이터를 받아 노드를 만드는 메서드**이므로, 이미 만든 `Node`를 인자로 전달하면 `Node.data` 안에 또 다른 `Node`가 들어가는 잘못된 중첩이 생긴다.

| 동작 | 해시맵과 LRU의 호출 흐름 |
| --- | --- |
| 새 키 `SET` | `StoreEntry` 생성 → `insert_front(entry)`가 LRU 노드 반환 → `HashMap.put(key, lru_node)` |
| 기존 키 `SET` | `HashMap.get(key)`으로 LRU 노드 조회 → `node.data.value` 갱신 → `move_to_front(node)` |
| 존재하는 키 `GET` | `HashMap.get(key)`으로 LRU 노드 조회 → `move_to_front(node)` → `node.data.value` 반환 |
| 없는 키 `GET` | 해시맵 조회 결과 `None` → LRU 변경 없이 `None` 반환 |
| 존재하는 키 `DEL` | 삭제 전에 LRU 노드 조회 → 해시맵에서 키 삭제 → `remove_node(lru_node)` |
| 없는 키 `DEL` | 해시맵 조회 결과 `None` → 두 구조 모두 변경 없이 `False` 반환 |

`DEL`에서는 해시맵의 `remove`가 삭제 성공 여부만 반환하므로, 해시맵에서 키를 제거하기 전에 LRU 노드를 먼저 얻어야 한다. 이후 자동 제거 단계에서는 `self._lru.tail.data.key`로 가장 오래된 키를 알아내고 해시맵에서도 같은 키를 제거할 수 있다. 해시맵의 평균 조회와 이미 찾은 노드의 이동·삭제를 조합하므로 LRU 리스트를 다시 순회할 필요가 없다.

`StoreEntry`에는 현재 6단계에 필요한 `key`와 `value`만 둔다. TTL 만료 시각을 엔트리에 둘지 별도 해시맵에 둘지는 8단계에서 결정할 사항이며, 아직 TTL 상태나 처리 코드를 추가하지 않는다.
...
**스스로 설명해 보기:** 해시맵 버킷의 `Node`와 LRU 리스트의 `Node`는 각각 무엇을 `data`로 가지는가? 해시맵만 있으면 최근 사용 순서를 어떻게 알 수 있으며, 연결 리스트만 있으면 임의의 키를 찾는 데 얼마나 걸리는가?
...
남은 시간의 소수 부분을 정수 초로 바꾸는 구체적인 반올림 규칙은 명세에 고정되어 있지 않다. 원문의 `EXPIRE ... 3` 직후 `TTL`이 2인 예시는 실행 지연과 소수 부분 버림 등으로 설명할 수 있으며, 모든 실행에서 반드시 2가 나와야 한다는 뜻은 아니다. 이번 구현은 `time.time()` 기반 절대 시각을 `float`로 저장하고, 양수인 남은 시간에 `int()`를 적용해 소수 부분을 버린다.

`EXPIRE`는 없는 키에 0, 정상 설정 시 1을 반환한다. 0 이하의 seconds는 즉시 만료로 처리해도 된다고 명시되어 있다. 이번 구현은 존재하는 키를 즉시 삭제하고 1을 반환하는 방식을 선택한다.

### 키 확인과 만료 후보 찾기

특정 키를 다루는 명령은 그 키가 만료되었는지 먼저 확인한다. 최소 힙은 여러 키 중 가장 이른 만료 후보를 찾는 데 사용한다. 이 두 역할이 함께 필요하다.

최소 힙을 보관하는 것만으로 시간이 지났을 때 코드가 자동 실행되지는 않는다. 이번 구현에서 키 기반 명령은 대상 키의 만료를 먼저 확인하고, `DBSIZE`·`KEYS`·`INFO memory`는 결과를 만들기 전에 힙에서 현재까지 만료된 항목을 정리한다. `CONFIG SET maxmemory`는 만료 정리를 일으키지 않으며 백그라운드 스레드는 추가하지 않는다.

### 오래된 TTL 기록은 현재 상태와 다를 수 있다

예를 들어 a의 만료 시각을 100으로 설정한 뒤, 만료 전에 200으로 다시 설정했다고 하자. 힙에서 100의 기록을 꺼냈다는 이유만으로 현재의 a를 삭제하면 안 된다.

이번 구현은 **지연 삭제(lazy deletion)**를 선택한다. 현재 유효한 만료 시각은 `StoreEntry.expire_at`에 두고, 최소 힙에는 `(expire_at, key)`를 저장한다. 힙 기록과 현재 엔트리의 만료 시각이 같은 경우만 실제 만료로 처리하고, 오래되어 효력을 잃은 기록은 버린다. `SET`의 TTL 초기화, `DEL` 후 같은 키 재생성, LRU 제거 후 재생성에서도 과거 기록이 현재 데이터를 잘못 삭제하지 않도록 해야 한다.

`DEL`의 “모든 구조에서 제거”와 지연 삭제 허용은 함께 읽어야 한다. TTL의 효력은 즉시 없애고, 힙에 남은 무효 기록은 나중에 정리한다. 키가 없거나 현재 `expire_at`이 `None` 또는 힙 기록과 다른 값이면 무효 기록이다. 단지 힙에 남겨 놓고 유효 여부를 확인하지 않는 것은 지연 삭제를 올바르게 구현한 것이 아니다.
...
`input()`으로 기다리는 동안 `Ctrl+C`를 누르면 `KeyboardInterrupt`, macOS·Linux에서 `Ctrl+D`로 입력 끝을 알리면 `EOFError`가 발생한다. 두 경우를 정상적인 REPL 종료 입력으로 선택했다면 `input()` 호출만 좁게 `try`로 감싸고, 두 예외에서는 줄을 정리한 뒤 반복을 끝낸다. 명령 실행 전체를 `Exception`으로 감싸면 자료구조 구현 오류까지 숨길 수 있으므로 서로 구분한다.

macOS·Linux의 `Ctrl+Z`는 Python 예외가 아니라 셸이 프로세스에 보내는 작업 일시정지 신호다. 보통 프로그램에서 잡지 않고 셸의 작업 제어에 맡기며, `fg`로 다시 실행할 수 있다. 이러한 중단 입력 처리는 원문 필수 조건이 아니라 CLI 사용성을 위한 선택적 내부 설계다.
...
현재 구현의 `_data`는 `key → LRU Node`를 저장하고 `_lru.tail` 하나만 보면 제거 후보를 알 수 있다. LFU로 바꾸면서 `StoreEntry`에 `frequency`만 추가하면 각 키의 횟수는 알 수 있지만, **가장 작은 횟수를 가진 키를 바로 찾을 수는 없다.** 제거 때마다 모든 엔트리를 순회하면 갱신은 간단하지만 제거 후보 탐색이 O(n)이 된다.

#### 평균 O(1)을 목표로 하는 빈도 버킷 구조

한 가지 설계는 다음 세 상태를 함께 관리하는 것이다. 실제 구현 시에는 과제 제약에 맞게 Python `dict` 대신 직접 만든 `HashMap`을 사용해야 한다.

- **키 조회 맵:** `key → LFUEntry` 또는 `key → 빈도 리스트의 Node`. 엔트리에는 `key`, `value`, `expire_at`, `frequency`와 현재 빈도 리스트에서의 노드 참조가 필요하다.
- **빈도 버킷 맵:** `frequency → DoublyLinkedList`. 같은 사용 횟수의 엔트리를 하나의 리스트로 묶는다. 현재 해시맵이 문자열 키만 받으므로 정수 빈도를 문자열로 변환하거나 해시맵의 키 타입을 일반화하는 설계 결정도 필요하다. 임의 삭제 뒤에도 다음 최소 빈도를 O(1)로 찾으려면 비어 있지 않은 빈도 버킷끼리 연결한 별도 정렬 리스트 같은 구조도 고려해야 한다.
- **최소 빈도:** 현재 저장된 키 중 가장 작은 횟수인 `min_frequency`. 제거 시 전체 빈도를 검색하지 않고 이 값의 리스트를 바로 찾는다.

빈도 버킷 안에서는 동률을 해결해야 한다. 각 리스트의 `head`를 그 빈도에서 가장 최근에 사용한 항목, `tail`을 가장 오래된 항목으로 두면 **사용 횟수가 같은 키 중 LRU인 키**를 제거할 수 있다.

호출별 상태 변화는 다음과 같다.

1. **새 키 `SET`:** 빈도를 1로 만들고 빈도 1 리스트의 앞에 넣은 뒤 `min_frequency = 1`로 갱신한다.
2. **성공한 `GET` 또는 기존 키 `SET`:** 키 맵으로 엔트리를 찾고, 빈도 `f` 리스트에서 기존 노드를 O(1)로 분리한다. 빈도를 `f + 1`로 올려 다음 리스트 앞에 넣는다.
3. **최소 빈도 보정:** 접근으로 옮긴 뒤 `f` 리스트가 비었고 `min_frequency == f`라면 방금 옮긴 키가 있는 `f + 1`로 올릴 수 있다. 반면 `DEL`·TTL 만료가 최소 리스트의 마지막 키를 없애면 다음 빈도가 `f + 1`이라고 보장할 수 없다. 빈도 숫자를 순회할지, 비어 있지 않은 빈도 버킷의 정렬 연결을 유지할지 선택해야 한다.
4. **메모리 초과 제거:** `min_frequency` 리스트의 `tail`을 제거한다. 키 맵, 빈도 리스트, TTL 효력, `used_memory`, `evicted_keys`를 현재 LRU 삭제 경로처럼 함께 갱신한다.
5. **`DEL`·TTL 만료:** 키 맵뿐 아니라 해당 빈도 리스트에서도 노드를 제거한다. 그 결과 최소 빈도 리스트가 비면 `min_frequency`를 다시 유효하게 만들어야 한다.

작은 예로 `a`와 `b`의 빈도가 각각 3과 1이면 최근 사용 시점과 관계없이 b가 먼저 제거된다. 둘 다 빈도 3이라면 빈도 3 리스트의 `tail`, 즉 같은 빈도 안에서 더 오래 사용하지 않은 키를 제거하는 식이다.

#### 빈도 노화와 정책 재설계

누적 횟수만 계속 증가시키면 초기에 많이 사용된 키가 이후 거의 쓰이지 않아도 오랫동안 제거되지 않는 **과거 인기 고착** 문제가 생긴다. 장시간 실행되는 저장소라면 다음 중 하나를 정책으로 정해야 한다.

- 일정 주기마다 모든 빈도를 절반으로 줄인다. 이해하기 쉽지만 한 번에 O(n) 갱신이 발생한다.
- 키마다 마지막 감쇠 시각을 두고 접근할 때 경과 시간에 맞춰 지연 감쇠한다. 전체 순회는 피하지만 시간 계산과 정책이 복잡해진다.
- 최대 빈도를 제한하거나 로그형 확률 카운터를 사용한다. 메모리와 범위는 제한되지만 정확한 접근 횟수와 달라진다.
- 새 정책으로 전환할 때 기존 LRU 데이터의 초기 빈도를 모두 1로 둘지, 과거 접근 기록을 변환할지도 결정해야 한다. 현재 구현에는 과거 횟수가 없으므로 정확한 복원은 불가능하다.

#### 구조별 비용 비교

| LFU 설계 | 접근 시 갱신 | 제거 후보 선택 | 추가 상태와 단점 |
| --- | --- | --- | --- |
| 엔트리에 횟수만 저장 | 평균 O(1) | O(n) 전체 순회 | 가장 단순하지만 eviction 순간이 느리다. |
| `(frequency, 순서, key)` 최소 힙 | O(log m) 새 기록 추가 | O(log m), 오래된 기록 검사 필요 | TTL처럼 무효 기록이 누적될 수 있다. |
| 키 맵 + 빈도별 이중 연결 리스트 + 최소 빈도 | 평균 O(1) | 평균 O(1) | 임의 삭제 후 최소 빈도까지 O(1)로 복구하려면 비어 있지 않은 빈도 버킷 순서도 관리해야 한다. |

따라서 LFU 전환은 LRU 노드에 카운터 하나를 붙이는 변경이 아니라, 제거 후보를 빠르게 찾기 위한 인덱스와 동률·노화 정책까지 다시 설계하는 작업이다.

### 12.2. 데이터가 10만 건이면 — 평가 4-2

병목은 실제 코드의 연산과 저장 형태를 기준으로 찾아야 한다. “10만 건이면 모두 느려진다”가 아니라 어떤 입력에서 어느 작업량이 커지고, 단일 스레드 REPL을 얼마나 오래 막는지 설명해야 한다.

#### 현재 구현에서의 구체적인 병목 후보

| 병목 후보 | 현재 코드에서 생기는 이유 | 10만 건에서 관찰할 영향 |
| --- | --- | --- |
| 해시 충돌 체인 | `_find_node`가 한 버킷의 연결 리스트를 실제 키가 나올 때까지 순회한다. | 평균 분산이 좋으면 짧지만 나쁜 해시 분포나 공격적 키 패턴에서는 조회·삭제가 O(n)에 가까워질 수 있다. |
| 한 번에 수행하는 재해싱 | `_resize`가 모든 기존 버킷과 엔트리를 한 호출에서 새 버킷으로 옮긴다. | 평소 요청은 빠르다가 임계값을 넘긴 한 번의 `SET`에서 O(n) 지연이 집중된다. |
| 빈 버킷 객체 수 | 현재 구현은 버킷마다 빈 `DoublyLinkedList` 객체를 미리 만든다. | 초기 용량 8, 임계값 0.75 규칙에서는 98,305번째 키에서 용량이 262,144로 늘어나므로 빈 버킷 객체와 참조의 메모리 비중이 커질 수 있다. |
| `KEYS` 수집과 출력 | `HashMap.keys()`는 모든 버킷 B와 엔트리 n을 순회하고, CLI는 다시 출력 문자열 목록을 만들고 `join`한다. | 시간은 O(B+n)이고 결과 배열·출력 문자열도 O(n) 추가 메모리를 사용한다. 터미널 출력 자체가 큰 병목이 된다. |
| TTL 무효 기록 누적 | TTL 재설정·`SET` 초기화·삭제 뒤에도 과거 `(expire_at, key)` 튜플은 힙에 남는 지연 삭제 방식이다. | 살아 있는 TTL 키보다 힙 원소 m이 훨씬 커져 `push`·`pop`의 log m 비용과 메모리가 증가할 수 있다. |
| 만료 폭주 | `DBSIZE`, `KEYS`, `INFO memory` 앞의 `_purge_expired`가 현재까지 만료된 기록을 반복해서 꺼낸다. | 동시에 k개가 만료되면 한 명령이 대략 O(k log m) 작업을 맡아 긴 지연을 만들 수 있다. |
| 대량 LRU 제거 | 큰 값을 저장한 뒤 제한을 맞추려고 `_evict_lru`를 반복 호출한다. | 한 번의 `SET`이 k개를 제거하면 평균 O(k) 삭제 작업과 문자열 바이트 계산을 수행한다. |
| 긴 키·값 | 해시 함수는 키 문자 수에 비례하고 `_entry_size`는 UTF-8 인코딩을 수행한다. 덮어쓰기에서는 기존 값과 새 값의 크기를 다시 계산한다. | 엔트리 개수뿐 아니라 문자열 총길이가 처리량과 임시 객체 생성량에 영향을 준다. |
| 단일 실행 흐름 | 현재 REPL은 한 명령이 끝난 뒤 다음 입력을 처리한다. 현재는 락이 없지만 동시 요청을 추가하면서 전역 쓰기 락 하나를 두면 같은 직렬화가 유지된다. | 재해싱·대량 만료·`KEYS` 하나가 다른 읽기와 쓰기를 모두 기다리게 만들 수 있다. |

`HashMap.size()`와 `_used_memory` 조회 자체는 현재 각각 저장된 카운터를 반환하므로 전체 순회 병목이 아니다. 대신 모든 삭제·만료·덮어쓰기 경로에서 이 카운터를 정확히 갱신해야 한다는 일관성 비용이 있다.

#### 개선 방향과 대가

- **해시 분산 측정과 개선:** 평균·최대 체인 길이를 기록하고 특정 키 패턴에서 분산을 확인한다. 해시식을 바꾸면 평균 충돌은 줄일 수 있지만 모든 최악 입력을 없앤다고 보장할 수는 없다.
- **점진적 재해싱:** 기존 테이블과 새 테이블을 잠시 함께 두고 명령마다 일정 개수의 버킷만 옮긴다. 긴 일회성 정지는 줄지만 조회가 두 테이블을 확인해야 하고, 삭제·갱신·확장 중 상태 관리가 복잡해진다.
- **지연 버킷 생성:** 빈 칸에는 `None`만 두고 실제 충돌이 생길 때 리스트를 만든다. 262,144개의 빈 리스트 객체 비용을 줄일 수 있지만 각 접근에서 빈 버킷 분기가 필요하다.
- **`KEYS` 페이지화 또는 스트리밍:** 커서 기반으로 일부 키만 반환하거나 출력 즉시 전달하면 한 번의 추가 메모리와 응답 지연을 줄일 수 있다. 다만 원문 `KEYS`처럼 전체 결과를 요구하면 총 O(n) 작업량과 출력량 자체는 사라지지 않는다.
- **TTL 힙 재구성:** `힙 기록 수 / 유효 TTL 키 수`가 임계값을 넘을 때 현재 유효한 TTL만으로 힙을 다시 만든다. 메모리는 회수하지만 재구성 순간 O(m) 비용이 생기므로 여러 명령에 나눠 수행할 수 있다.
- **만료·제거 작업 예산:** 명령 하나가 정리할 최대 개수를 정하고 남은 작업을 다음 명령이나 백그라운드 작업으로 넘긴다. 긴 지연은 줄지만 `DBSIZE`·`KEYS`가 반드시 최신 상태를 보여야 하는 현재 규칙과 충돌하지 않도록 관찰 시점 정책을 다시 정해야 한다.

#### 샤딩과 비동기화

규모와 동시 요청이 더 커지면 하나의 `Store`를 여러 샤드로 나누는 방법을 고려할 수 있다. 키 해시의 일부로 샤드를 고르고, 각 샤드가 독립된 해시맵·LRU 리스트·TTL 힙·메모리 카운터를 가지면 서로 다른 샤드의 요청을 병렬로 처리할 수 있다.

하지만 샤딩은 다음 정책을 새로 요구한다.

- 전역 `maxmemory`를 샤드별 고정 예산으로 나눌지, 샤드 간 여유를 이동할지 정해야 한다.
- 전역에서 가장 오래된 키 하나를 정확히 고르기 어렵기 때문에 샤드별 LRU 또는 여러 샤드 후보 중 샘플링하는 근사 정책이 필요할 수 있다.
- `KEYS`, `DBSIZE`, `INFO memory`는 모든 샤드의 결과를 모아야 한다.
- 인기 키가 한 샤드에 몰리면 부하 불균형이 생긴다.

만료 정리, 점진적 재해싱, 큰 결과 출력은 작은 작업 단위로 나눠 이벤트 루프나 백그라운드 작업에 맡길 수 있다. 다만 비동기 I/O만 도입한다고 해시 계산·재해싱 같은 CPU 작업이 자동으로 빨라지는 것은 아니다. Python에서 병렬 CPU 처리가 필요하다면 스레드의 GIL 영향, 프로세스 간 데이터 분리와 통신 비용까지 비교해야 한다.

#### 먼저 측정할 지표

개선 전에는 평균값만 보지 말고 명령 지연의 p95·p99, 최대 체인 길이, 재해싱 시간, `KEYS` 결과 바이트 수, 유효 TTL 수 대비 힙 기록 수, 한 명령이 정리한 만료·eviction 개수, 프로세스 실제 메모리를 측정해야 한다. 측정 결과가 병목을 가리킨 뒤에 점진적 재해싱·힙 재구성·샤딩 같은 복잡한 설계를 선택하는 것이 타당하다.

위 개선안은 평가 답변을 위한 설계 비교이며 현재 필수 구현 목록에는 추가하지 않는다.

### 12.3. 자료구조 오버헤드까지 포함한다면 — 평가 4-3

현재 `used_memory`는 다음 논리적 payload 크기만 센다.

```text
Σ(len(key.encode("utf-8")) + len(value.encode("utf-8")))
```

이 모델은 구현 언어와 자료구조가 달라도 같은 키·값이면 같은 점수를 얻는 장점이 있다. 실제 프로세스 메모리와는 다르지만 과제의 LRU 제거 결과를 재현하기 쉽다.

#### 오버헤드를 포함할 때 추가되는 항목

현재 객체 관계를 기준으로 하면 다음 비용을 검토해야 한다.

- `StoreEntry` 객체와 `key`, `value`, `expire_at` 속성 저장 공간
- 해시맵의 `HashEntry`와 체이닝 버킷 `Node`
- LRU 리스트의 별도 `Node`
- 각 노드의 `prev`, `next`, `data` 참조
- 버킷 배열의 참조 슬롯과 버킷마다 미리 만든 `DoublyLinkedList` 객체
- 최소 힙의 리스트 용량, 각 `(expire_at, key)` 튜플과 시각 객체
- TTL 재설정·삭제 뒤 아직 힙에 남아 있는 무효 기록
- 동적 배열이 실제 원소 수보다 넉넉하게 확보한 여유 용량
- 해시맵 확장 도중 잠시 함께 존재하는 이전·새 버킷 배열의 **최대 순간 사용량**

같은 키 문자열은 `StoreEntry`, `HashEntry`, TTL 튜플에서 참조될 수 있다. 객체 본체를 참조 횟수만큼 다시 더하면 중복 계산이 된다. 반대로 컨테이너의 얕은 크기만 재면 컨테이너가 참조하는 객체는 빠진다.

#### 두 가지 보정 모델

1. **표준 가상 단가 모델**
   - payload 바이트에 `StoreEntry 1개`, `LRU Node 1개`, `HashEntry 1개`, `버킷 Node 1개`, `버킷 슬롯`, `TTL 기록` 등의 고정 단가를 더한다.
   - 모든 제출물에 같은 단가표를 적용하므로 OS와 Python 버전 차이를 줄일 수 있다.
   - 실제 CPython 메모리와 정확히 같지는 않지만 채점 결과를 재현하기 쉽다.
2. **실제 객체 그래프 측정 모델**
   - `Store`에서 도달 가능한 소유 객체를 순회하고 객체 정체성 `id`를 기록해 한 객체를 한 번만 센다.
   - `sys.getsizeof`는 얕은 크기만 반환하므로 자식 객체를 별도로 순회해야 한다. 문자열 객체 크기에 문자 데이터가 포함되는 환경이라면 UTF-8 payload를 다시 더하지 않도록 규칙을 정해야 한다.
   - 클래스·모듈·인터프리터처럼 모든 인스턴스가 공유하는 전역 비용을 어디까지 포함할지도 정해야 한다.

실제 메모리 제한으로 바꾸면 `SET`의 OOM과 LRU 제거 시점도 달라진다. 예를 들어 짧은 키·값은 payload보다 노드 오버헤드가 더 클 수 있고, 해시맵 확장만으로 제한을 넘을 수도 있다. 무효 TTL 기록을 물리적으로 남겨 두면 논리적으로 삭제된 키도 메모리를 계속 차지한다. 따라서 다음 두 방식 중 하나가 필요하다.

- 모든 할당·해제·버킷 확장·힙 기록 변경 시 증감분을 반영한다. 조회는 빠르지만 공유 객체와 배열 용량 변화까지 정확히 갱신하기 어렵다.
- 필요할 때 객체 그래프를 다시 순회해 계산한다. 구현은 한곳에 모을 수 있지만 매번 O(객체 수) 비용이 들고 제거 반복 중 재측정 비용이 커진다.

#### 동일 기준 비교와 채점 원칙

- **포함 범위 고정:** payload, 문자열 객체, 엔트리, 노드, 참조 슬롯, 빈 버킷, 배열 여유 공간, 유효·무효 TTL 기록 중 무엇을 셀지 먼저 공개한다.
- **객체 정체성 기준 중복 제거:** 같은 객체를 여러 구조가 참조해도 본체는 한 번만 세고, 각 컨테이너가 가진 참조 슬롯 비용은 해당 컨테이너 비용에 포함한다.
- **빈 저장소 기준값:** 저장소를 막 만든 상태의 기본 비용을 포함할지, 각 구현의 측정값에서 빼고 증가분만 비교할지 통일한다.
- **동일 환경:** Python 구현·버전, 32/64비트 구조, OS, 메모리 할당기와 측정 도구를 맞춘다.
- **동일 데이터와 명령 이력:** 같은 키·값뿐 아니라 TTL 재설정 횟수, 삭제, 재해싱 시점까지 같은 명령 순서를 실행한다. 명령 이력이 달라지면 무효 힙 기록과 배열 용량이 달라진다.
- **측정 시점 고정:** 명령 직후의 정상 상태, garbage collection 뒤의 정상 상태, 연산 중 최대치 중 무엇을 비교할지 정한다. 평균 메모리와 최대 순간 메모리는 별도 지표로 보고할 수 있다.
- **반복 측정과 허용 오차:** 실제 프로세스 메모리는 실행마다 달라질 수 있으므로 여러 번 측정한 중앙값과 사전에 정한 허용 오차를 사용한다.
- **논리량과 물리량 분리:** `logical_used_memory`와 `physical_or_estimated_memory`를 함께 보고하면 정책 결과와 구현 효율을 혼동하지 않고 비교할 수 있다.

공정한 과제 채점에는 동일 결과를 재현하기 쉬운 현재 payload 공식이나 공개된 가상 단가표가 적합하다. 실제 운영 최적화에는 객체 그래프·프로세스 메모리와 최대 순간 사용량이 더 유용하다. 현재 구현의 `used_memory`는 원문 공식대로 유지하며, 이 평가 질문 때문에 동작을 변경하지 않는다.
